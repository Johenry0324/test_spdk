...
static void usage(const char *prog)
{
	...
	printf("  -f <pct> partial percent (1-100, default 50)\n");
...
int main(int argc, char **argv)
{
	...
	struct zone_opts zopts = { .open_zones = 8, .pattern_id = 0, .percent = 50 };
	...
	while ((op = getopt(argc, argv, "o:p:f:d:i:r:h")) != -1) {
		switch (op) {
		case 'o':
		...
		case 'p':
			...
			break;
		case 'f':
			zopts.percent = spdk_strtol(optarg, 10);
			break;
		case 'd':
	...
	printf("zone_partial_write: total_lba=%lu zone_size_lba=%lu chunk_lba=%u open_zones=%d percent=%d pattern=%d\n",
	       zopts.total_lba, zopts.zone_size_lba, zopts.chunk_lba, zopts.open_zones, zopts.percent, zopts.pattern_id);

	rc = zone_partial_write(ns, qpair, zones, zone_count, zopts.chunk_lba, zopts.open_zones, zopts.percent, zopts.pattern_id, &stat);
	if (rc != 0) {
		fprintf(stderr, "partial_write failed\n");
	}
