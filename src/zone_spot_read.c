...
int main(int argc, char **argv)
{
	...
	printf("zone_spot_read: total_lba=%lu zone_size_lba=%lu pattern=%d\n",
	       zopts.total_lba, zopts.zone_size_lba, zopts.pattern_id);

	rc = zone_spot_read_verify(ns, qpair, zones, zone_count, zopts.pattern_id, &stat);
	if (rc != 0) {
		fprintf(stderr, "spot_read failed rc=%d\n", rc);
	}
