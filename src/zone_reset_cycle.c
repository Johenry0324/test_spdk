...
static void usage(const char *prog)
{
	...
	printf("  -p <id>  first pattern id (default 0)\n");
	printf("  -q <id>  second pattern id (default 1)\n");
...
int main(int argc, char **argv)
{
	...
	while ((op = getopt(argc, argv, "o:p:q:d:i:r:h")) != -1) {
		switch (op) {
		case 'o':
		...
		case 'p':
			...
			break;
		case 'q':
			pattern2 = spdk_strtol(optarg, 10);
			break;
		case 'd':
	...
	printf("zone_reset_cycle: write(pattern %d) -> reset -> write(pattern %d) -> verify(pattern %d)\n",
	       zopts.pattern_id, pattern2, pattern2);
	...
	zone_reset_all(zones, zone_count);

	rc = zone_seq_write(ns, qpair, zones, zone_count, zopts.chunk_lba, zopts.open_zones, pattern2, &stat);
	if (rc != 0) {
		fprintf(stderr, "second write failed\n");
		goto out;
	}

	rc = zone_full_read_verify(ns, qpair, zones, zone_count, zopts.chunk_lba, pattern2, &stat);
	if (rc != 0) {
